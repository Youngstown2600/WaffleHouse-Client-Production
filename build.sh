...
Build WaffleHouse-Client 2.5.1, the unified C++ GUI/CLI executable.
...
show_header() {
  cat <<EOF2
============================================================
              WAFFLEHOUSE-CLIENT 2.5.1 + SIP SOFTPHONE
============================================================
Host OS:        $HOST_OS
Build jobs:     $JOBS
...
echo
echo "==> Configuring WaffleHouse-Client 2.5.1"
...
echo
echo "==> Building WaffleHouse-Client 2.5.1"
run_cmd cmake --build build --parallel "$JOBS"

if [ "$INSTALL_MODE" = yes ]; then
  echo
  echo "==> Installing WaffleHouse-Client 2.5.1"
  prepare_privileges
...
if [ "$DRY_RUN" -eq 1 ]; then echo "Dry run complete."; else echo "WaffleHouse-Client 2.5.1 build complete."; fi
