...
Build WaffleHouse-Client 2.5.2, the unified C++ GUI/CLI executable.
...
show_header() {
  cat <<EOF2
============================================================
              WAFFLEHOUSE-CLIENT 2.5.2 + SIP SOFTPHONE
============================================================
Host OS:        $HOST_OS
Build jobs:     $JOBS
...
echo
echo "==> Configuring WaffleHouse-Client 2.5.2"
...
echo
echo "==> Building WaffleHouse-Client 2.5.2"
run_cmd cmake --build build --parallel "$JOBS"

if [ "$INSTALL_MODE" = yes ]; then
  echo
  echo "==> Installing WaffleHouse-Client 2.5.2"
  prepare_privileges
...
if [ "$DRY_RUN" -eq 1 ]; then echo "Dry run complete."; else echo "WaffleHouse-Client 2.5.2 build complete."; fi
