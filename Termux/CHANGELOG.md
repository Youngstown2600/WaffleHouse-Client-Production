# Build 1.1 — Termux current-call /hangup

## Build 1.2
- Added explicit `asterisk-chan_sip` compatibility mode while retaining PJSIP 2.17 as the client SIP stack.
- Legacy mode disables RFC 5626 SIP-Outbound, uses legacy Contact rewriting, wildcard Digest-realm authentication, and correct caller-ID/AOR domain handling.
- SIP compatibility selection is available in the Termux add/edit account form and persists with the account.


- Termux-only behavior: `/hangup` with no argument now hangs up the current foreground SIP call.
- If no foreground flag is present and exactly one live call exists, `/hangup` safely targets that call.
- With multiple live calls and no foreground call, WaffleHouse refuses to guess and asks for `/hangup CALL-ID`.
- `/hangup CALL-ID` remains fully supported for explicit multi-call control.
- Desktop Linux/Unix/macOS WaffleHouse behavior is unchanged.
- Retains Build 1.0 automatic microphone preflight, responsive TUI, and known-good SIP/RTP/audio path.

# Build 1.0 — Automatic Android microphone preflight

- Bakes `wafflehouse-audio-preflight` into normal client startup.
- Successful microphone permission/capture checks are silent.
- Missing permission invokes Termux:API microphone capture, allowing Android to surface its runtime permission dialog.
- Failed permission checks explain recovery and then continue startup; SIP retains Build 0.8+ speaker-only/null-audio fallback.
- Standalone `wafflehouse-audio-preflight` remains available for manual diagnostics.
- Retains Build 0.9 responsive terminal resizing and all prior SIP/RTP/audio fixes.

## Build 1.0 — Responsive Termux TUI
