...
void TerminalUi::requestClientVersion(ConnectionEntry *entry, QString target)
{
    ...
    QTimer::singleShot(3500, this, [this, key, target, protocol] {
        ...
        status(protocol == ConnectionSettings::Protocol::Oscar
            ? QStringLiteral("[version] %1: no Build 1.0 reply; peer may be an older WaffleHouse/CPX client or another AIM client (exact version unavailable)").arg(target)
...
void TerminalUi::handleCommand(const QString &line)
{
    ...
    if (command == QStringLiteral("hangup")) {
        ...
        if (rest.trimmed().isEmpty()) {
            const auto callList = m_sipController->calls();
            for (const auto &call : callList) {
                if (!call.disconnected && call.foreground) { id = call.id; break; }
            }
            if (id < 0) {
                int onlyLiveId = -1;
                int liveCount = 0;
                for (const auto &call : callList) {
                    if (call.disconnected) continue;
                    onlyLiveId = call.id;
                    ++liveCount;
                }
                if (liveCount == 1) id = onlyLiveId;
                else if (liveCount == 0) { status(QStringLiteral("No active call to hang up.")); return; }
                else { status(QStringLiteral("Multiple active calls; use /hangup CALL-ID.")); return; }
            }
        } else {
            id = parseCallId(rest, QStringLiteral("Usage: /hangup [CALL-ID]"));
            if (id < 0) return;
        }
        QString error; if (!m_sipController->hangup(id, &error)) status(QStringLiteral("Hangup failed: %1").arg(error));
        else status(QStringLiteral("Hung up call %1.").arg(id));
        showPhoneCalls(true); return;
    }
...
void TerminalUi::showHelp()
{
    const QStringList lines = {
        ...
        QStringLiteral("  /hangup [ID]                 end current call (or specified call)"),
                                          ...
                                          bool editing)
{
    ...
    QList<FormField> fields = {
        ...
        {QStringLiteral("sipcompat"), QStringLiteral("SIP compatibility (auto/standard/asterisk-chan_sip)"), settings.sipCompatibility.isEmpty() ? QStringLiteral("auto") : settings.sipCompatibility, FieldType::Text},
    ...
    while (true) {
        ...
        if (currentProtocol == ConnectionSettings::Protocol::Sip) {
            ...
            try {
                ...
                messageBox(QStringLiteral("SIP Account"),
                           {QStringLiteral("SIP compatibility must be auto, standard, or asterisk-chan_sip.")});
                continue;
            }
        }
        bool expOk = false;
