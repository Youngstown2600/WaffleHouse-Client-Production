...
- Rebuilt directly from the supplied 5.0r8 desktop bundle, preserving its newer desktop/macOS baseline fixes.
- Added per-account SIP server compatibility: `auto`, `standard`, and `asterisk-chan_sip`.
- Legacy `asterisk-chan_sip` mode keeps PJSIP 2.17 client-side while using conservative registration/NAT behavior for older Asterisk `chan_sip` servers.
- GUI and CLI expose and persist the same compatibility setting.
- Caller-ID/AOR domain can differ from the registrar domain without being discarded.
