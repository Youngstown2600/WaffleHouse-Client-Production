...
Build WaffleHouse-Client 5.0r10 for macOS. The .app contains the GUI and the same
...
# building. yt-dlp is not used by the 5.0r10 media implementation.
...
DEPLOY_ARGS=""
if [ "$MAKE_DMG" -eq 1 ]; then DEPLOY_ARGS="-dmg"; fi
# shellcheck disable=SC2086
"$MACDEPLOYQT" "$APP" $DEPLOY_ARGS
...
if [ "$INSTALL_MODE" = yes ]; then
  echo
  echo "==> Installing WaffleHouse-Client 5.0r10 for macOS"
...
WaffleHouse-Client 5.0r10 macOS build complete.
GUI: open "$APP"
