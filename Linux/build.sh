...
usage() {
  cat <<'USAGE'
WaffleHouse-Client 5.0r10 — combined desktop builder
...
if [ -z "$SELECTED_OS" ]; then
  ...
  cat <<'PROMPT'
============================================================
             WAFFLEHOUSE-CLIENT 5.0r10 DESKTOP
============================================================
...
case "$SELECTED_OS:$HOST_OS" in
  ...
  freebsd:*)
    echo "You selected FreeBSD / Unix, but this host reports '$HOST_OS'. The validated Unix target in 5.0r10 is FreeBSD." >&2
    exit 2
    ;;
  *)
