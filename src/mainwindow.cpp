...
private:
    ...
    void updateFields()
    {
        ...
        m_telnetColumnsLabel->setVisible(telnet);
        m_telnetColumns->setVisible(telnet);
        m_telnetRowsLabel->setVisible(telnet);
        m_telnetRows->setVisible(telnet);
        m_telnetAutoFit->setVisible(telnet);
...
void MainWindow::executeGuiCommand(const QString &input)
{
    ...
    if (!BuildFeatures::Sip && sipCommands.contains(command)) {
        report(QStringLiteral("SIP/VoIP support was not included in this build."));
        return;
    }
    ...
    if (command == QStringLiteral("autopresence")) {
        ...
        if (action.isEmpty()) {
            report(QStringLiteral("Native OSCAR idle reporting: %1 | auto-away: disabled (server/manual Away only)")
        ...
        if (action == QStringLiteral("on") || action == QStringLiteral("off")) {
            ...
            if (!m_options.autoPresenceEnabled) markUserActivity();
            saveOptions();
            report(QStringLiteral("Native OSCAR idle reporting %1.").arg(action.toUpper()));
            return;
        }
    ...
    if (command == QStringLiteral("setprofile")) {
        ...
        oscar->setProfile(rest); state->aimProfile = rest; saveConnections(); return;
    }
