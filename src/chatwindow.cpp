...
void ChatWindow::showEvent(QShowEvent *event)
{
    ...
    if (m_initialInputFocusApplied || !m_messageEdit) {
        return;
    }
    m_initialInputFocusApplied = true;

    // showEvent happens before the window manager/Qt finish activating a new
    // top-level window.  Queue the focus assignment so the message composer
    // wins after that activation work, rather than the read-only transcript.
    QTimer::singleShot(0, this, [this] {
        if (m_messageEdit && isVisible()) {
