...
check "Build 1.2 branding" "grep -q 'Build 1.2' '$ROOT/src/appbranding.h'"
...
check "chan_sip compatibility profile" "grep -q 'asterisk-chan_sip' '$ROOT/src/sipcore/Profile.cpp' && grep -q 'PJSUA_CONTACT_REWRITE_UNREGISTER' '$ROOT/src/sipcore/SipEngine.cpp' && grep -q 'sipOutboundUse=false' '$ROOT/src/sipcore/SipEngine.cpp'"
...
check "Termux current-call hangup" "grep -q 'rest.trimmed().isEmpty()' '$ROOT/src/terminalui.cpp' && grep -q 'call.foreground' '$ROOT/src/terminalui.cpp' && grep -q 'Multiple active calls; use /hangup CALL-ID.' '$ROOT/src/terminalui.cpp'"
...
check "Automatic startup audio preflight" "grep -q 'runStartupAudioPreflight' '$ROOT/src/main_termux.cpp' && grep -q 'wafflehouse-audio-preflight' '$ROOT/src/main_termux.cpp' && grep -q -- '--startup' '$ROOT/src/main_termux.cpp'"
check "Startup preflight never blocks client on denial" "grep -q 'WaffleHouse will continue' '$ROOT/scripts/termux-audio-preflight.sh' && grep -q 'continuing without blocking startup' '$ROOT/src/main_termux.cpp'"
check "Successful startup preflight is silent" "grep -q 'MODE=startup' '$ROOT/scripts/termux-audio-preflight.sh' && grep -q '== manual' '$ROOT/scripts/termux-audio-preflight.sh'"
...
check "Softphone version updated" "grep -q 'Build 1.2' '$ROOT/include/trunkmonkey/Version.h'"
