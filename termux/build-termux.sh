...
APP="WaffleHouse-Client-Termux Build 1.2"
...
for arg in "$@"; do
  case "$arg" in
    --clean|--pjsip|--test|--uninstall|--help|-h) ;;
...
$APP builder
  ...
  ./build.sh --install     build and install into Termux $PREFIX/bin
  ./build.sh --no-install  build only; do not add WaffleHouse to $PREFIX/bin
  ./build.sh --uninstall   remove installed WaffleHouse files

Without --install/--no-install, the builder asks after a successful compile.
Pressing Enter at the prompt means No.
USAGE
  exit 0
fi
if [[ " ${*:-} " == *" --uninstall "* ]]; then
...
Provides: xdg-utils
Conflicts: xdg-utils
Description: Metadata-only xdg-utils provider for WaffleHouse-Client-Termux Build 1.2
...
pjsip_limits_ok(){
  ...
  # Native Termux has no Java VM. Build 1.2 replaces PJSIP's Android/JNI
...
install_now="$INSTALL_MODE"
if [[ "$install_now" == ask ]]; then
  if [[ -t 0 ]]; then
    printf '\nInstall WaffleHouse-Client into %s/bin and shared Termux data now? [y/N]: ' "$PREFIX"
    IFS= read -r answer
    case "$answer" in y|Y|yes|YES|Yes) install_now=yes ;; *) install_now=no ;; esac
  else
    install_now=no
  fi
fi
...
Android microphone preflight: automatic at client startup
Manual microphone retest: wafflehouse-audio-preflight
DONE
else
  cat <<DONE

$APP build complete; installation was skipped.
Built executable: $BUILD/wafflehouse-client
To install later: ./build.sh --install
DONE
fi
